...
class FullRisersConfig(BasePACSConfig):
    ...
    def add_full_riser(self, riser: PACS.FullRiser, level: int = None, height: float = None) -> None:
...
class RowRisersConfig(BasePACSConfig):
    ...
    def __init__(self, enabled: bool = True, row_risers: List[PACS.RowRiser] = []) -> None:
        super().__init__(enabled)
        self.row_risers = []
        if row_risers:
    ...
    def add_row_riser(self, riser: PACS.RowRiser, level: int = None, row: int = None) -> None:
        ...
        if riser:
            ...
            assert (riser.level not in levels) or (riser.row not in rows), "Row riser at level %s and row %s already exists" % (riser.level, riser.row)
            self.row_risers.append(riser)
        else:
            assert (level not in levels) or (row not in rows), "Row riser at level %s and row %s already exists" % (level, row)
