    ...
    def test_serial_number(self):
        ...
        for serial_number in INVALID_SERIAL_NUMBERS:
            try:
                ...
                errors.append("Invalid Serial Number %s was incorrectly accepted" % serial_number)
        # Valid Inputs
        for serial_number in VALID_SERIAL_NUMBERS:
            try:
                self.pfmconfig.set_serial_number(serial_number)
            except AssertionError as e:
                errors.append("Valid Serial Number %s was incorrectly rejected with the following message: %s" % (serial_number, e.args[0]))
            if serial_number != self.pfmconfig.get_serial_number() and serial_number != self.pfmconfig.get_serial_number(True):
                errors.append("Valid Serial Number %s was incorrectly set as %s" % (serial_number, self.pfmconfig.get_serial_number()))
...
class TestDecorationsConfig():
    ...
    def test_bumper(self):
        ...
        # Extension
        errors.extend(invalid_object_variable_check(
            init_test=lambda extension: Decorations.Bumper(extension=extension),
            set_test=lambda obj, extension: Decorations.Bumper.set_extension(obj, extension),
            invalid_entries=INVALID_EXTENSIONS
        ))
        errors.extend(valid_object_variable_check(
            init_test=lambda extension: Decorations.Bumper(extension=extension),
            set_test=lambda obj, extension: Decorations.Bumper.set_extension(obj, extension),
    ...
    def test_a200_top_plate(self):
        ...
        errors.extend(invalid_object_variable_check(
            ...
            set_test=lambda obj, model: Decorations.A200.TopPlate.set_model(obj, model),
...
VALID_FULL_RISERS=[[PACS.FullRiser(1), PACS.FullRiser(2, 0.2), PACS.FullRiser(3, 0.2)]]
# RowRisers
INVALID_ROW_RISERS=[[PACS.RowRiser(1,1), PACS.RowRiser(1,1)]]
VALID_ROW_RISERS=[[PACS.RowRiser(1,1, 0.1), PACS.RowRiser(1, 2, 0.2), PACS.RowRiser(2, 2, 0.2)]]
# Brackets
...
class TestPACSConfig():
    ...
    def test_full_risers_config(self):
        ...
        errors.extend(invalid_object_variable_check(
            ...
            set_test=lambda obj, full_risers: FullRisersConfig.set_full_risers(obj, full_risers),
    ...
    def test_row_risers_config(self):
        ...
        errors.extend(invalid_object_variable_check(
            ...
            set_test=lambda obj, row_risers: RowRisersConfig.set_row_risers(obj, row_risers),
    ...
    def test_brackets_config(self):
        ...
        errors.extend(invalid_object_variable_check(
            ...
            set_test=lambda obj, brackets: BracketsConfig.set_brackets(obj, brackets),
