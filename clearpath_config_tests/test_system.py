...
INVALID_HOSTNAMES = ["space in hostname",
                    ...
                    "-start-hypens-end-",
                    "trailing.period.",
                    "exceedingly----------long----------names-----------which----------are----------longer----------than----------two----------hundred----------and----------fifty----------three----------ASCII----------characters----------are----------not-----------allowed----------as----------hostnames",
                    ]
    ...
    def test_platform(self):
        ...
        for hostname in VALID_HOSTNAMES:
            try:
                ...
                errors.append("Valid Hostname %s was incorrectly rejected with following message:\n\t%s" % (hostname, e.args[0]))
            if hostname != self.hosts.get_platform_hostname():
                errors.append("Valid Hostname %s was incorrectly set as %s" % (ip, self.hosts.get_platform_hostname()))
        for ip in VALID_IP:
            try:
                self.hosts.set_platform_ip(ip)
            except AssertionError as e:
                errors.append("Valid IP %s was incorrectly rejected with following message:\n\t%s" % (ip, e.args[0]))
            if ip != self.hosts.get_platform_ip():
                errors.append("Valid IP %s was incorrectly set as %s" % (ip, self.hosts.get_platform_ip()))
        # Duplicate Inputs
        ...
        try:
            ...
            errors.append("Valid Hostname and IP %s were incorrectly rejected with the following message:\n\t%s" % ((hostname, ip), e.args[0]))
        if hostname != self.hosts.get_platform_hostname() or ip != self.hosts.get_platform_ip():
            errors.append("Valid Hostname and IP %s were incorrectly set as %s" % ((hostname, ip), self.hosts.get_platform()))
        # Set Value Again
        try:
            self.hosts.set_platform(Host(hostname, ip))
        except AssertionError as e:
            ...
            errors.append("Duplicate Hostname '%s' and IP '%s' accepted." % (hostname, ip))
        ...
        for hostname in VALID_HOSTNAMES:
            for ip in VALID_IP:
                ...
                try:
                    ...
                    errors.append("Unable to remove existing hostname and ip with following message:\n\t%s" % e.args[0])
                for host in self.hosts.get_onboard():
                    if hostname == host.get_hostname() or ip == host.get_ip():
                        errors.append("Remove failed. Host '%s','%s' was not removed" % (host.get_hostname(), host.get_ip()))
        assert not errors, "Errors: %s" % "\n".join(errors)
